...
def simulate_leapfrog_step(ps: ParticleSystem, sim: SimulationParameters): 
    #This is the added code to simulate a leapfrog-algorithm step, in the NVE ensemble
    """
    ...
    **Outside of this function in LJ_gas_run_MD.py**
    ...
    A_step(ps, sim, half_step = False)      # update position by a full time step
    calculate_force(ps, sim)                # udpate force
    print(np.max(np.linalg.norm(ps.force, axis=1)))
